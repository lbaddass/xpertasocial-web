...
export default function GrowthSystemsPage() {
  const plans = [
    ...
    },
    {
      ...
      includes: [
        "20 Piezas (8 Reels + 12 Estáticos)",
        "Funnel Express Incluido",
        "Gestión de campañas PPC",
        "Reporte de ROI Detallado",
      ],
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-8 border-b border-white/10 pb-12">
          <div className="space-y-4">
            <div className="inline-block px-3 py-1 bg-neonLime/10 border border-neonLime/20 rounded-full">
              <span className="text-xs font-mono text-neonLime tracking-widest uppercase">
                PISO 3: GROWTH SYSTEMS
              </span>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight">
              MARKETING{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-neonLime to-green-500">
                24/7
              </span>
            </h1>
            <p className="text-xl text-white/60 max-w-2xl font-light">
              Sin contratos forzosos. La consistencia genera confianza. Tu
              equipo de marketing externo.
            </p>
          </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`group relative bg-carbon/50 border p-8 transition-all duration-300 overflow-hidden flex flex-col ${
                plan.id === "ventas"
                  ? "border-neonLime/50 shadow-[0_0_30px_-10px_rgba(132,204,22,0.2)]"
                  : "border-white/10 hover:border-neonLime/30"
              }`}
            >
              {plan.id === "ventas" && (
              ...
              <div className="relative z-10 space-y-6 flex-1 flex flex-col">
                <div className="space-y-2">
                  ...
                  <div className="text-sm font-mono text-white/50 uppercase tracking-wider">
                    {plan.focus}
                  </div>
                </div>
                ...
                <ul className="space-y-4 flex-1 py-4">
                  {plan.includes.map((feature, idx) => (
                    ...
                    >
                      <Check
                        size={16}
                        className="text-neonLime mt-0.5 shrink-0"
                      />
                      {feature}
                    </li>
                  ))}
        ...
        <div className="bg-carbon border border-white/10 p-8 md:p-12 rounded-lg flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 space-y-6">
            ...
            <h2 className="text-3xl font-bold text-white">
              No somos un freelancer que desaparece.
            </h2>
            <p className="text-white/60 leading-relaxed">
              Al contratar un sistema, obtienes acceso a un equipo completo:
              Estratega, Copywriter, Diseñador y Media Buyer. Todo coordinado
              por nuestros Project Managers para que tú solo apruebes y vendas.
            </p>
          </div>
