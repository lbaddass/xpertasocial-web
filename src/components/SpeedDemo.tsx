...
export default function SpeedDemo() {
  ...
  return (
    <section
      id="speed-demo"
      className="py-20 bg-[#1f2234] text-white border-b border-white/10"
    >
      <div className="max-w-6xl mx-auto px-6 md:px-10 lg:px-16 grid lg:grid-cols-2 gap-8 items-center">
        <div>
          <p className="text-[#E63946] font-mono text-xs tracking-[0.3em]">
            TU SITIO VS NUESTRA INFRA
          </p>
          <h2 className="text-3xl font-black tracking-tight mb-3">
            Benchmark de carga.
          </h2>
          <p className="text-white/60 font-mono text-sm mb-6">
            Simulamos la diferencia entre un stack Wordpress saturado y nuestra
            infraestructura Next.js + Edge caching.
          </p>
          <button
            className="inline-flex items-center gap-2 px-5 py-3 rounded-lg border border-white/20 text-white font-semibold hover:border-[#E63946] transition-colors"
            aria-label="Probar mi velocidad actual"
          >
...
function DemoCard({
  ...
  return (
    <div
      className={`relative border border-white/10 rounded-xl bg-[#2B2D42] p-5 overflow-hidden ${accentClass}`}
    >
      ...
      <p className="mt-2 text-xs font-mono text-white/60">
        {stuck
          ? "Se atasca en 72% (bloqueado por plugins)"
          : "Edge cache + optimización de assets"}
      </p>
    </div>
  );
}
